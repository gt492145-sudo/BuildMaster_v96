(function (global) {
    ...
    const RULES = [
    {
        ...
        "msg": "這一組是第1到3頁的智慧量測工具：先做智慧定比例，再做智慧量圖，之後尺寸會回填到右側計算欄位。"
    },
    {
    ...
    {
        "sel": "#memberChatQuickInput') || target.closest('.member-chat-quick-send",
        "key": "memberChatQuickInput",
        "msg": "群組大廳：輸入文字快速送出（本機泡泡）；試算完成也會自動推送📊卡片到此頻道。"
    },
    {
        "sel": "#memberChatPanel') || target.closest('#memberChatMessageList",
        "key": "memberChatPanel",
        "msg": "會員聊天：可加入好友、切換對象；文字與試算卡片皆存於本機。公開隱私說明見 Google Sites／privacy.html。"
    },
    {
        "sel": "#mobileFuncTab') || target.closest('#mobileFuncDrawer",
        "key": "mobileFuncTab",
        "msg": "手機功能抽屜：集中 3D、量測、日照等戰術工具。"
    },
    {
        "sel": "#mobileLeftTab') || target.closest('#mobileLeftDrawer",
        "key": "mobileLeftTab",
    ...
    const HINTS = {
        'zh-TW': {
            'coach.hint.calcMeasureCluster': "這一組是第1到3頁的智慧量測工具：先做智慧定比例，再做智慧量圖，之後尺寸會回填到右側計算欄位。",
            ...
            'coach.hint.stakeExecutionCluster': "這一組是第四頁放樣執行設定：先勾選柱、牆、梁與放樣高精度，再執行一鍵放樣流程或 IBM 雲端放樣。",
            'coach.hint.stakeQaCluster': "這一組是第四頁放樣 QA 檢核：集中做控制點配準、偏差熱圖、穩定度重測、分群與放樣 QA。",
            'coach.hint.stakeExportCluster': "這一組是第四頁放樣輸出與現場工具：完成 QA 後再匯出放樣點、QA 報告、施工包，或開啟補點建議與現場抽驗。",
            'coach.hint.ifcInput': "這裡上傳模型檔，系統會做 BIM QA 解析與構件統計。",
            ...
            'coach.hint.generateBimLayoutPoints': "從模型自動抽取放樣點（柱心、牆端點、梁端點）。",
            ...
            'coach.hint.fileInput': "這格是圖紙上傳框：先選圖片，再做定比例與量測。",
            ...
            'coach.hint.coachToggle': "可在這裡一鍵開關解說員；開啟後點擊任何功能區都會出現說明，含群組聊天、試算卡片與隱私權對照提示。",
            ...
            'coach.hint.workCalcBtn': "計算模式：固定對應第1到3頁，包含工種試算、智慧量圖、AI 看圖辨識、IBM 自動估算與報表輸出。",
            'coach.hint.workStakeBtn': "放樣模式：固定對應第4頁，包含模型解析、放樣點抽取、控制點配準、放樣 QA 與施工包輸出。",
            'coach.hint.workElectricalBtn': "電機模式：設定電壓／電流／腳本模式，一鍵匯出 mecha_config.txt 供本機 Python 或現場測試（僅存裝置）。",
            ...
            'coach.hint.coachGuideBtn': "點這裡可重跑新手導覽，含第1頁聊天、試算卡片泡泡與隱私權說明。",
            'coach.hint.contrastToggle': "高對比模式：加強文字與按鈕對比，夜間或戶外較好辨識。",
            'coach.hint.contrastAutoToggle': "自動高對比：傍晚到清晨自動切換，白天恢復一般模式。",
            'coach.hint.calcPage1Btn': "第1頁：簡單試算＋本機群組聊天。聊天與試算📊卡片為裝置本機示範，非雲端即時多人；詳見公開隱私權（Google Sites／privacy.html）。",
            'coach.hint.calcPage2Btn': "第2頁：圖面量測與完整工程功能（含進階試算、IBM 工具）。",
            ...
            'coach.hint.freeWarRoomCard': "第1頁交流區：群組大廳為本機泡泡對話；按「吸入計算清單」後，試算結果會自動變成📊卡片泡泡。資料僅存本機，換裝置不同步。",
            ...
            'coach.hint.calcType': "工種公式選擇區：不同工種會套不同計算公式。",
            ...
            'coach.hint.previewBar': "即時預覽區：顯示目前算出的數量與金額。",
            'coach.hint.shareCalcChatBtn': "將最近一次試算結果以📊卡片泡泡再送到群組大廳（本機示範，非雲端廣播）。",
            'coach.hint.btnAdd': "主按鈕：把目前資料加入計算清單，並自動同步試算📊卡片到群組大廳（本機聊天，不上傳雲端）。",
            'coach.hint.listBody': "明細清單：可檢查每筆數量、單價與金額。",
            'coach.hint.btnExport': "匯出按鈕：下載 Excel/CSV 報表。",
            ...
            'coach.hint.drawingPanel': "左側是圖紙操作區：上傳、定比例、量測都在這裡。",
            'coach.hint.calcPanel': "右側是主控制區：計算模式只顯示第1到3頁內容，放樣模式只顯示第四頁內容；兩邊現在已分開顯示。",
        },
        en: {
            'coach.hint.calcMeasureCluster': "Pages 1–3 smart measure: set smart scale, then smart measure; dimensions fill the calc fields on the right.",
            ...
            'coach.hint.coachToggle': "Toggle coach; tap any area for tips on chat, cards, and privacy.",
            ...
            'coach.hint.drawingPanel': "Left: blueprint upload, scale, measure.",
            'coach.hint.calcPanel': "Right: calc mode pages 1–3; stake mode page 4 only.",
        },
        ja: {
            ...
            'coach.hint.coachToggle': "解説員オン／オフ。各エリアタップで説明。",
